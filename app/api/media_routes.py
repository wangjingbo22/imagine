...
def _media_storage_unavailable() -> AppError:
    return AppError(
...
@contextmanager
def _media_connection(request: Request) -> Iterator[sqlite3.Connection]:
    try:
        with _connection(request) as connection:
            yield connection
    except sqlite3.Error:
        raise _media_storage_unavailable() from None


def _initialize(request: Request) -> None:
    with _media_connection(request) as connection:
