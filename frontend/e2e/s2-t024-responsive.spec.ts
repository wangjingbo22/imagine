...
test('mocked single-person UI integration confirms the recommendation and enters the workspace', async ({ page }) => {
  ...
  await expect(page.getByText('行程执行中', { exact: true })).toBeVisible()
...
for (const fixture of [
  ...
  test(`mocked ${fixture.kind} fixture stays responsive (UI contract only)`, async ({ page }) => {
    ...
    if (fixture.kind === 'execute') {
      ...
      await expect(page.getByText('调整类型', { exact: true })).toHaveCount(0)
      await expect(page.getByRole('button', { name: '疲劳', exact: true })).toHaveCount(0)
      await expect(page.getByText('疲劳程度', { exact: true })).toHaveCount(0)
