...
export function MemberConversationPage() {
  ...
  return <AppShell compact><main className="planner-layout member-planner-layout"><section className="planner-panel motion-enter">
    ...
    {proposalPanelOpen && <section className="change-proposal-card" aria-labelledby="change-proposal-title">
      ...
      {proposalDrafts.length > 0 && <div className="proposal-draft-list"><strong>待发送修改清单</strong><ol>{proposalDrafts.map((item) => <li key={item.fieldPath}><span>{proposalFields.find((field) => field.value === item.fieldPath)?.label}：{item.fieldPath === 'trip.budgetCents' && typeof item.proposedValue === 'number' ? `¥${item.proposedValue / 100}` : String(item.proposedValue)}</span><small>{item.reason}</small><button type="button" aria-label="从修改清单移除" onClick={() => setProposalDrafts((items) => items.filter((draft) => draft.fieldPath !== item.fieldPath))}>移除</button></li>)}</ol></div>}
      {proposalNotice && <p className="proposal-notice" role="status">{proposalNotice}</p>}
      {view.changeProposals.length > 0 && <ol className="change-proposal-list">{[...view.changeProposals].reverse().map((item) => <li key={item.proposalId}><div><strong>{proposalFields.find((field) => field.value === item.fieldPath)?.label ?? item.fieldPath}</strong><span>建议值：{item.fieldPath === 'trip.budgetCents' && typeof item.proposedValue === 'number' ? `¥${item.proposedValue / 100}` : String(item.proposedValue)}</span><p>{item.reason}</p>{item.organizerNote && <small>组织者说明：{item.organizerNote}</small>}</div><em className={`proposal-status proposal-status--${item.status.toLowerCase()}`}>{item.status === 'PENDING' ? '待组织者审核' : item.status === 'APPROVED' ? '已批准并执行' : '已拒绝，保留原计划'}</em></li>)}</ol>}
    </section>}
