...
重新推荐只更新候选方案。用户点击“接受推荐并确认 Plan V1”后才进入执行状态。
...
当前后端已经确认的契约是 S1-T001 `CreateSingleDayTrip`。
...
页面中的步行、换乘和休息设置属于 UI 草稿，等待后端 T003 扩展正式 Schema。
...
- 多人入口尚未开放
- 正式 HTTP URL 尚未登记
- AssistanceProfile 尚未进入后端 Trip Schema
- 页面刷新后 Mock 执行状态会重置
- Mock 素材刷新后会丢失
- 导出为 HTML，不是 PDF
